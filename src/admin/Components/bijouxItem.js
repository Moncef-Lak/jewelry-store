...
const BijouxElem = ({id,name,description,category_id,image_name,price,active,image_name2,index,setItemID,setIsPageHidden,setItemImg,setItemImgBack}) => {
    
    const {categoryItems}=useGlobalContext();
    const [imgUrl,setImgUrl]=useState(null);
    const [backImgUrl,setBackImgUrl]=useState(null);
    const category=categoryItems.filter(category=>category.id===category_id);
    ...
    return (  
        <tr className={`${index%2>0 && 'cd'}`}>
            ...
            <td className='img'><img onClick={e=>e.target.classList.toggle('zoomImg')} src={backImgUrl} alt='img'/></td>
            <td>{name}</td>
            <td>{category[0] ? category[0].name : 'Unknown' }</td>
            <td>{description.length >20 ?  description.substring(0,19)+"...":description}</td>
            <td>{active==='Yes' ?<SiOrigin className='active-yes'/>: <MdDoNotDisturbAlt className='active-no'/>}</td>
            <td>{price}$</td>
            <td>
                <Link to='/bijoux' className='delete' onClick={()=>{setItemID(id);setIsPageHidden(false);setItemImg(image_name);setItemImgBack(image_name2)}}><IoIosRemoveCircle className='delete-in'/></Link> 
                <Link to={`/update-bijoux/${id}`} className='update'><MdUpdate className='update-in'/></Link>
            </td>
        </tr>
    );
}
