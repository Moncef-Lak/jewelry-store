...
const Detail = React.memo(() => {
  ...
  const getMoreBijoux = useCallback(() => {
    ...
    try {
      axios.post(process.env.REACT_APP_API_PASSWORD+'getMoreBijouxItem.php', myObj)
        .then(data => {
          if (data.data !== 'Error') {
            setFourBijoux(data.data.moreItems);
            setSameCatagory(data.data.sameCategoryProduct);
            const { name, description, image_name, price, image_name2, id } = data.data.sameCategoryProduct;
            setState2({ name, description, price, image_name, image_name2, id });
            try {
              if (require('../images/bijoux-image/' + image_name)) {
                setFrImgUrl(require('../images/bijoux-image/' + image_name).default)
              }
            } catch (error) {
            ...
            try {
              if (require('../images/bijoux-image/' + image_name2)) {
                setFrBackImgUrl(require('../images/bijoux-image/' + image_name2).default)
              }
            } catch (error) {
              setFrBackImgUrl(errorImage)
            }
            if (bijouxItem[0]) {
              const { name, description, image_name, price, image_name2 } = bijouxItem[0];
              setState({ ...state, name, description, price, image_name, image_name2 })
            }
        
            try {
              if (require('../images/bijoux-image/' + bijouxItem[0].image_name)) {
                setImgUrl(require('../images/bijoux-image/' + bijouxItem[0].image_name).default)
              }
            } catch (error) {
              setImgUrl(errorImage)
            }
            try {
              if (require('../images/bijoux-image/' + bijouxItem[0].image_name2)) {
                setBackImgUrl(require('../images/bijoux-image/' + bijouxItem[0].image_name2).default)
              }
            } catch (error) {
      ...
      <section className='details-page'>
        <div className='in-deails'>
          ...
          <div className='details-side'>
            ...
            <button onClick={() => addItem(state.name, state.price, imgUrl, id)}>Add to cart</button>
          </div>
        ...
        {sameCatagory &&
          <div className='more-product-1'>
            <div className='left-fitt'>
              <h2>See also</h2>
              <h1>{state2.name}</h1>
              <Link to={`/detail/${state2.id}`}><button>BUY ONLINE</button></Link>
            </div>
        ...
        <div className='more-product-2'>
          ...
          {fourBijoux.map((item, index) => {
            return <MoreItem {...item} key={index} shiffre={id} fourBijoux={fourBijoux} />
          })}
